...
func alterTable()  {
	//_, err := db.Exec("ALTER TABLE test.hello ADD COLUMN id int(10)")
	_, err := db.Exec("ALTER TABLE test.hello ADD COLUMN id int(10) PRIMARY KEY AUTO_INCREMENT")
	if err != nil{
		log.Fatalln(err)
	}
}
...
func insert() {
	rs, err := db.Exec("INSERT INTO test.hello(world) VALUES ('wqqqq')")
	if err != nil{
		log.Fatalln(err)
	}
